...
import uk.gov.justice.digital.hmpps.keyworker.services.QueueAttributes.*
...
class QueueHealth(@Autowired @Qualifier("awsSqsClient") private val awsSqsClient: AmazonSQS,
  ...
  override fun health(): Health {
    val queueAttributes = try {
      awsSqsClient.getQueueAttributes(getQueueAttributesRequest(awsSqsClient.getQueueUrl(queueName)))
    } catch (e: Exception) {
  ...
  private fun Health.Builder.addDlqHealth(mainQueueAttributes: GetQueueAttributesResult): Health.Builder {
    ...
    val dlqAttributes = try {
      awsSqsDlqClient.getQueueAttributes(getQueueAttributesRequest(awsSqsDlqClient.getQueueUrl(dlqName)))
